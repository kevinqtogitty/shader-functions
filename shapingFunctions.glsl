...
void main(void)
{
    ...
    float time = u_time/2.0; //slow down time
  	...
  	vec2 uv = -1.0 + 5.0 * normCoord;
  
  	uv.x *= u_resolution.x/u_resolution.y; //to get perfect circles

	float radius = length(uv * 7.0);
  
  	float rings = mod(sin(time - radius), .9);
  
  	float angle = sin(atan(uv.x , uv.y) - time);
  
   float swirly = abs(sin(rings - cos(angle) + time));

   float noise = cnoise(vec3(radius, (angle) * 3.0, swirly));
  
    vec4 color = vec4((.6), atan(noise), noise, noise * time);
    gl_FragColor = color;
}